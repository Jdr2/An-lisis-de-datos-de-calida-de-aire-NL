...
```{r}
library(readxl)
datos=read_excel('DatosFinales_LIMPIOS_2022_2023_Normales.xlsx',sheet=2)
head(datos)
```
...
heatmap(cor(mi_data))
```
Como podemos observar, la mayoría de los VIF son menores a 5, lo cual es bueno. Sin embargo, PM10 tiene un VIF de 18, lo cual es muy alto, por lo que hay multicolinealidad con el PM10 y se cuestiona la confiabilidad de los resultados de nuestra regresión logística multinomial. En conclusión, este supuesto no se cumple.
...
plot(predict(model))
```
Observamos que la mayoría de las predicciones del model son para la categoría 2 = Aceptable.
